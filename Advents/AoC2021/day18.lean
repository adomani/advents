...
def tall (dat : Array String) : snail :=
  let f := dat[0]!
  let fs : snail := [.i f.getNats[0]!, .i f.getNats[1]!]
  (dat.erase f).foldl (init := fs) fun t (n : String) =>
    let ns : List Nat := n.getNats
    reduce (t + reduce ([.i ns[0]!, .i ns[1]!] : snail))
...
[6,6]"]
  for dat in dats do
    let dat := dat.splitOn "\n"
    IO.println <| tall dat.toArray

#eval do
  let dat ← IO.FS.lines input
  let f := dat[0]!
  let fs : snail := [.i f.getNats[0]!, .i f.getNats[1]!]
  let tot := (dat.erase f).foldl (init := fs) fun t (n : String) =>
    let ns : List Nat := n.getNats
    reduce (t + reduce ([.i ns[0]!, .i ns[1]!] : snail))
  IO.println tot
  IO.println <| magnitude tot
