...
def part1 (dat : Array String) : Nat :=
  let ls := ((dat.map String.ep)).map part1.tot
  ls.sum
...
abbrev memo := HashMap (List springs × List Nat) Nat

def doAll : memo → List springs → List Nat → Nat × memo
  | x,          l,         [] =>
    if l.all fun cs => cs.all (· == .q) then
      (1, x.insert (l, []) 1) else (0, x.insert (l, []) 0)
  | x,         [],          r => (0, x.insert ([], r) 0)
  | x, lh@(l::ls), rh@(n::ns) =>
    if ls = [] ∧ ! l.contains .h then
      let e1 := evalOne l (n::ns)
      (e1, x.insert (lh, rh) e1) else
    match doOne n l with
    | none => (0, x.insert (lh, rh) 0)
    | some nls =>
      Id.run do
      let mut nx := x
      let mut news := 0
...
--      let news := nls.map fun nl =>
        if nl = [.s] then
          let mem := x.find? (ls, rh)
          if mem.isSome then
            news := news + mem.get!
          else
            let da := doAll x ls rh
            news := news + da.1
            nx := da.2.insert (ls, rh) da.1
        else
          let mem := x.find? (nl::ls, ns)
          if mem.isSome then
            news := news + mem.get!
          else
            let da := doAll x (nl::ls) ns
            news := news + da.1
            nx := da.2.insert (nl::ls, ns) da.1
--          doAll x (nl::ls) ns
      --dbg_trace news
      (news, nx)
...
#assert
  ...
  for i in [:10] do
    ...
    vals := (doAll .empty [ls] [n, 1, 1]).1 :: vals
...
#assert
  ...
  let actual := (range 12).map fun i => (doAll .empty [replicate (n+i) .q] <| replicate i 1).1
...
#assert
  ...
  let fir := t.ep
  let lr := t.reparseOne
  let da := doAll .empty lr.1 lr.2
  da.1 == 0 && part1.tot fir == da.1

#assert  -- `atest` yields 21
  let tots := atest.map fun t =>
    let t := repl t 1
    let lr := t.reparseOne
    (doAll .empty lr.1 lr.2).1
  tots == #[1, 4, 1, 1, 4, 10]
...
def count (s : String) (f : String → String := id) : Nat := let (l, r) :=
  (f s).reparseOne; (doAll .empty l r).1
...
partial
def combine (mem : memo) (l : List springs) (ns : List Nat) : Nat × memo :=
  if l.length ≤ 1 then doAll mem l ns else
  let arr := doTwo l ns
  Id.run do
  let mut newMem := mem
  let mut arrparts := #[]
...
--  let arrparts := arr.map fun (x, xs) =>
    let (doa, nmem) := doAll newMem [x.1] x.2
    newMem := nmem
    if doa = 0 then
--      newMem := newMem.insert ([x.1], x.2) 0
      arrparts := arrparts.push 0 else
    let c := (combine newMem xs.1 xs.2)
    newMem := newMem.insert (x.1::xs.1, xs.2) (doa * c.1)
    arrparts := arrparts.push (doa * c.1)
  (arrparts.sum, newMem)

def count2 (mem : memo) (s : String) (f : String → String := id) : Nat × memo := let (l, r) :=
  (f s).reparseOne; combine mem l r

#eval show MetaM _ from do
--  let dat := (← IO.FS.lines input)[1]!
  let dat := "???.? 1,1"
  IO.println dat
  IO.println <| (count dat, (count2 .empty dat).1)

#eval do
--  let dat ← IO.FS.lines input
--  for d in dat do IO.println <| repl d
  let (val, mem) := count2 .empty "?##????????????##????????????##????????????##????????????##?????????? 2,2,2,2,2,2,2,2,2,2,2,2,2,2,2"
  IO.println <| val
...
#eval show MetaM _ from do
  ...
  let mut memo := .empty
  for t in dat do
    let oldmem := memo.size
--  let tota := dat.map (count  · (repl · 1))
    let next := count2 memo t (repl · 2)
    memo := next.2
    let ms := memo.size
    if oldmem = ms then
      IO.println s!"{ms}, {t}"
    else if ms < oldmem then IO.println "bad"
    tot2 := tot2 + next.1
...
def memos (l : List springs) (ns : List Nat) : HashMap (springs × List Nat) Nat :=
  ...
  let arr := doTwo l ns
  for (x, _) in arr do
    if ! mem.contains x then mem := mem.insert x (doAll .empty [x.1] x.2).1
...
def combine1' (l : List springs) (ns : List Nat) :=
  if l.length ≤ 1 then (doAll .empty l ns).1 else
  let mem := memos l ns
  mem.size
...
/-
-- repl 1
interpretation of _eval took 125ms
6935
...
-- repl 4
interpretation of _eval took 5.83e+03s
10799907995
-/
...
def second (dat : Array String) (i : Nat := 5) : Array Nat :=
  Id.run do
  let mut total := #[]
  for t in dat do
    let (l, r) := (repl t i).reparseOne
    let da := doAll (l.map String.toList) r
    total := total.push da
  total

#assert second atest == #[1, 16384, 1, 16, 2500, 506250]
--#assert second atest == #[1, 16384, 1, 16, 2500, 506250]
--#assert second atest == #[1, 16384, 1, 16, 2500, 506250]
--#assert second atest == #[1, 16384, 1, 16, 2500, 506250]
